...
import (
	...
	blspubkeyreg "github.com/ExocoreNetwork/exocore-sdk/contracts/bindings/BLSPubkeyRegistry"
	blsregistrycoordinator "github.com/ExocoreNetwork/exocore-sdk/contracts/bindings/BLSRegistryCoordinatorWithIndices"
	iblssigchecker "github.com/ExocoreNetwork/exocore-sdk/contracts/bindings/IBLSSignatureChecker"
...
func run(args []string) {
	...
	app.Description = "Prints all reachable exocore and AVS contract addresses starting from any one contract."
...
func printAddrs(c *cli.Context) error {
	...
	exocoreContractAddrs, err := getExocoreContractAddrs(client, registryCoordinatorAddr)
	if err != nil {
		return err
	}
...
func getRegistryCoordinatorAddr(c *cli.Context, client *ethclient.Client) (common.Address, error) {
	...
	if serviceManagerAddrString != "" {
		...
		// However, exoaddrs is targetted at teams using our registry contracts, so we assume that they are using our registries
...
func getExocoreContractAddrs(client *ethclient.Client, registryCoordinatorAddr common.Address) (map[string]string, error) {
	blsRegistryCoordinatorWithIndicesC, err := blsregistrycoordinator.NewContractBLSRegistryCoordinatorWithIndices(registryCoordinatorAddr, client)
	if err != nil {
		return nil, err
	}
