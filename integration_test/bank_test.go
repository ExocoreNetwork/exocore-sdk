...
import (
	"encoding/json"
	"fmt"
	"github.com/ExocoreNetwork/exocore-sdk/x/bank"
	"math/rand"
	"sync"
	"time"
...
func multiSend(s IntegrationTestSuite) {
	...
	coins, err := types.ParseDecCoins("10atom")
	s.NoError(err)
	...
	coins, err = types.ParseDecCoins("1atom")
	s.NoError(err)
	to := s.GetRandAccount().Address.String()
	begin := time.Now()
...
func sendWitchSpecAccountInfo(s IntegrationTestSuite) {
	for i := 0; i < 10; i++ {
		...
		amount, _ := types.ParseDecCoins("10atom")
