...
import (
	...
	sdkelcontracts "github.com/ExocoreNetwork/exocore-sdk/chainio/exocontracts"
...
type PubkeyCompendiumServiceInMemory struct {
	exocoreSubscriber sdkelcontracts.EXOSubscriber
	exocoreReader     sdkelcontracts.EXOReader
...
func NewPubkeyCompendiumInMemory(
	...
	pkcs := &PubkeyCompendiumServiceInMemory{
		exocoreSubscriber: exocoreSubscriber,
		exocoreReader:     exocoreReader,
		logger:            logger,
		queryC:            queryC,
	}
...
func (pkcs *PubkeyCompendiumServiceInMemory) startServiceInGoroutine(ctx context.Context, queryC <-chan query, wg *sync.WaitGroup) {
	go func() {
		...
		newPubkeyRegistrationC, newPubkeyRegistrationSub, err := pkcs.exocoreSubscriber.SubscribeToNewPubkeyRegistrations()
		if err != nil {
...
func (pkcs *PubkeyCompendiumServiceInMemory) queryPastRegisteredOperatorEventsAndFillDb(pubkeydict map[common.Address]types.OperatorPubkeys) {
	...
	alreadyRegisteredOperatorAddrs, alreadyRegisteredOperatorPubkeys, err := pkcs.exocoreReader.QueryExistingRegisteredOperatorPubKeys(nil, nil)
	if err != nil {
